...
from xbotg.modules.sql import languages_sql as sql
...
for x in os.listdir('xbotg/modules/langs'):
	if os.path.isdir('xbotg/modules/langs/'+x):
		continue
	x = x.replace('.py', '')
	LOADED_LANGS_ID.append(x)
	imported_langs = importlib.import_module("xbotg.modules.langs." + x)
