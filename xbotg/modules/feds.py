...
from xbotg import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, SARDEGNA_USERS, GBAN_LOGS, LOGGER
from xbotg.modules.helper_funcs.chat_status import is_user_admin
from xbotg.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from xbotg.modules.helper_funcs.string_handling import markdown_parser
from xbotg.modules.disable import DisableAbleCommandHandler

import xbotg.modules.sql.feds_sql as sql
...
def new_fed(bot: Bot, update: Update):
    ...
    if fednam != '':
        ...
        if not x:
            message.reply_text(
                "Can't federate! Please contact @X_Newbie if the problem persist.")
            return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @X-Newbie should this problem persist!")
            return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @X_Newbie.")
            return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in @X_Newbie !")
            return
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "xbotg_fbanned_users.json"
                message.reply_document(
                    document=output,
                    filename="xbotg_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "xbotg_fbanned_users.csv"
                message.reply_document(
                    document=output,
                    filename="xbotg_fbanned_users.csv",
